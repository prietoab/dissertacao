...
\section{O Programa permitirá novos estudos}
...
\section{$f_i = e^{-\lambda(\nabla\rho_i)^2}$ não leva ao mínimo}
	...
	\section{$f_i = e^{-\lambda(\rho_i - \rho_0)^2}$ leva ao mínimo, mas devemos saber aproximadamente onde ele está}
	...
	\begin{enumerate}
		\item \textbf{O que acontece quando $\rho_0$ está um pouco acima do mínimo?}
		
				Para com $<\rho> \approx \rho_0$
				
				Não encontra o menor autovalor.
				
				Porém, verifica-se que o $\nabla \rho \approx 0$.
				
				Então, estamos próximos ao menor autovalor.
				
		\item \textbf{O que acontece quando $\rho_0$ está um pouco abaixo do mínimo?}
						
				Encontra o mínimo.
				
				$<fitness>$ é próximo a 1 pois $<\rho>$ é próximo de $\rho_0$.
						
		\item \textbf{O que acontece quando $\rho_0$ está muito acima do mínimo?}
		
				A escolha para o $\lambda$ se torna mais delicada. (preciso verificar)
				...
				Portanto, estamos distantes do menor autovalor.
					
		
		\item \textbf{O que acontece quando $\rho_0$ está muito abaixo do mínimo?}
...
\section{$f_i = e^{-\lambda(\rho_i - \rho_0)^2}$ próximo de $\rho$ é intrinsicamente impreciso}
	...
	Gráfico comparando $f$ com $df/d\rho$. Verificar visualmente que $df/d\rho$ é muito menor.
	...
	Para fitness muito pequeno há o mesmo problema. Com isso, pode-se definir uma região do fitness onde a função é apropriada para o uso dos algoritmos genéticos. Figura \ref{fitness_boaRegiao}.
